...
Meanwhile, failed allocations still count towards the size of the pool, so there are only so many exceptions that the pool can contain.
...
This, at least in principle, prevents the blocking methods from waiting for forever. Stormpot only has two blocking methods: the `claim` method on the `Pool` interface, and the `await` method on the `Completion` interface mentioned in the previous section.
