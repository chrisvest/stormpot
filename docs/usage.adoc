...
The objects that you store in a Stormpot pool needs to implement the +Poolable+ interface. The absolute minimum amount of code required for its implementation is this:
...
These are thread-safe, and can easily be put in +static final+ constants.
...
Stormpot does no leak detection, so if you loose the reference to an object that hasn't been released back into the pool, it will not come back on its own.
