...
When you instantiate a `Pool` instance, the constructor will take the monitor-lock on the `Config` instance, thereby preventing it from being modified while they are looking at it.
The pool will then call the `validate` method to make sure that they have a `Config` instance that can be used.
