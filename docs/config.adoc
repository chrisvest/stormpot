...
The +Expiration+ policy is configured with +setExpiration+.
...
An object that has been allocated is assumed to have a unique identity among its peers in the pool.
An object that has been deallocated is assumed to be gone forever.
...
You can read more about how Stormpot through JMX in the link:jmx.html[JMX Guide].
...
In such a case it's nice that the API for awaiting on the completion of the shut down process, requires one to provide a timeout, so there's no waiting forever.

=== Complete Config Source

For reference, the complete source for the +Config+ class is listed bellow.
...
This help ensure a uniform recording of some of the metrics, across +Pool+ implementations.
