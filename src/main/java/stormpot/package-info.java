...
<p>The object pools themselves implement the {@link stormpot.Pool} interface,
...
pools.</li>
<li>Stormpot only has one type of pool, with extension interfaces, whereas
Commons-Pool has two types, where the implementations may expose additional
implementation specific APIs.</li>
...
<p>Now that we have a class of objects to pool, we need some way to tell Stormpot
...
<span class="ccn">Pool</span><span class="cco">&lt;</span><span class="ccn">MyPoolable</span><span class="cco">&gt;</span> <span class="ccn">pool</span> <span class="cco">=</span> <span class="cck">new</span> <span class="ccn">BlazePool</span><span class="cco">&lt;</span><span class="ccn">MyPoolable</span><span class="cco">&gt;(</span><span class="ccn">config</span><span class="cco">);</span>
<span class="ccn">Timeout</span> <span class="ccn">timeout</span> <span class="cco">=</span> <span class="cck">new</span> <span class="ccn">Timeout</span><span class="cco">(</span><span class="ccmi">1</span><span class="cco">,</span> <span class="ccn">TimeUnit</span><span class="cco">.</span><span class="ccna">SECONDS</span><span class="cco">);</span>

<span class="ccn">MyPoolable</span> <span class="ccn">object</span> <span class="cco">=</span> <span class="ccn">pool</span><span class="cco">.</span><span class="ccna">claim</span><span class="cco">(</span><span class="ccn">timeout</span><span class="cco">);</span>
<span class="cck">try</span> <span class="cco">{</span>
  <span class="ccc1">// do stuff with &#39;object&#39;</span>
<span class="cco">}</span> <span class="cck">finally</span> <span class="cco">{</span>
  <span class="cck">if</span> <span class="cco">(</span><span class="ccn">object</span> <span class="cco">!=</span> <span class="cckc">null</span><span class="cco">)</span> <span class="cco">{</span>
    <span class="ccn">object</span><span class="cco">.</span><span class="ccna">release</span><span class="cco">();</span>
  <span class="cco">}</span>
<span class="cco">}</span>
</code></pre></div>
...
never complete normally. You can still just halt the JVM, though.</p>
...
{@link stormpot.bpool.BlazePool BlazePool}.</p>
...
<span class="cckn">import</span> <span class="ccnn">stormpot.bpool.BlazePool</span><span class="cco">;</span>
</code></pre></div>
...
<p>We could now just create a <code>Config</code>, set the allocator, create a <code>BlazePool</code>
...
<div class="codehilite"><pre><code>  <span class="cckd">static</span> <span class="cckd">class</span> <span class="ccnc">MyDaoPool</span> <span class="cco">{</span>
    ...
    <span class="cckd">public</span> <span class="ccnf">MyDaoPool</span><span class="cco">(</span><span class="ccn">DataSource</span> <span class="ccn">dataSource</span><span class="cco">)</span> <span class="cco">{</span>
      ...
      <span class="ccn">pool</span> <span class="cco">=</span> <span class="cck">new</span> <span class="ccn">BlazePool</span><span class="cco">&lt;</span><span class="ccn">MyDao</span><span class="cco">&gt;(</span><span class="ccn">config</span><span class="cco">);</span>
    <span class="cco">}</span>
</code></pre></div>

<p>The set-up is simple: The <code>DataSource</code> goes into our <code>Allocator</code>, the <code>Allocator</code>
into a <code>Config</code>, and the <code>Config</code> into a <code>Pool</code> implementation - <code>BlazePool</code> in
