...
The library will take care of creating and destroying your objects in the background.

Stormpot is very mature, is used in production, and has done over a trillion footnote:[Fermi estimate. The benchmark that produced the charts for the 2.1 release blog post accounts for about 400 billion of those cycles.] claim-release cycles in testing.
...
There are a number of options out there, when it comes to object pools on the JVM. Stormpot has been carefully designed for high performance, and robust
...
Stormpot only depends on Java 6 or newer. Add it as a Maven dependency to your projects:
 ...
 * *Improve the documentation* by all means! Just fork the project and start.
   ...
   The website and javadocs can be generated with +mvn generate-resources javadoc:javadoc+.
